import { RGBA, type RenderContext, type SelectionState } from '../types'
import { OptimizedBuffer } from './browser-buffer'
import { Renderable } from '../Renderable'
import { GroupRenderable, FrameBufferRenderable, type FrameBufferOptions, type StyledTextOptions, StyledTextRenderable } from '../objects'
  ...
  private setupMouse(): void {
    ...
    this.canvas.addEventListener('mousemove', (e) => {
      ...
      if (this.lastOverRenderable) {
        const event = { x, y, type: 'move' as const, button: 0, modifiers: { shift: e.shiftKey, alt: e.altKey, ctrl: e.ctrlKey }}
  ...
  private findRenderableAt(x: number, y: number): Renderable | undefined {
    ...
    for (const [, renderable] of Renderable.renderablesByNumber) {
      if (renderable.visible && 
          x >= renderable.x && x < renderable.x + renderable.width &&
          y >= renderable.y && y < renderable.y + renderable.height) {
        return renderable
      }
    }
    return undefined
  ...
  private drawBufferToCanvas(): void {
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        // Draw background
        if (cell.bg) {
          this.ctx.fillStyle = `rgba(${cell.bg.r}, ${cell.bg.g}, ${cell.bg.b}, ${cell.bg.a / 255})`
          this.ctx.fillRect(px, py, CHAR_WIDTH, CHAR_HEIGHT)
        }
        
        // Draw character
        if (cell.fg) {
          this.ctx.fillStyle = `rgba(${cell.fg.r}, ${cell.fg.g}, ${cell.fg.b}, ${cell.fg.a / 255})`
