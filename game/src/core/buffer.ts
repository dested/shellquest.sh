  ...
  public setCellWithAlphaBlendingLocal(
    ...
    if (hasBgAlpha || hasFgAlpha) {
      ...
      if (destCell) {
        ...
        const preserveChar =
          char === ' ' && destCell.char !== 0 && String.fromCharCode(destCell.char) !== ' ';
  ...
  public drawFrameBufferLocal(
    ...
    for (let dY = startDestY; dY <= endDestY; dY++) {
      for (let dX = startDestX; dX <= endDestX; dX++) {
        ...
        if (
          frameBuffer.buffer.bg[srcColorIndex + 3] === 0 &&
          frameBuffer.buffer.fg[srcColorIndex + 3] === 0
        ) {
          continue;
        }

        const charCode = frameBuffer.buffer.char[srcIndex];
        const fg: RGBA = RGBA.fromArray(
          frameBuffer.buffer.fg.slice(srcColorIndex, srcColorIndex + 4),
        );
        const bg: RGBA = RGBA.fromArray(
          frameBuffer.buffer.bg.slice(srcColorIndex, srcColorIndex + 4),
        );
