import {BufferedElement, type ElementOptions} from '../element.ts';
import type {CliRenderer} from '../../index.ts';
import {parseColor} from '../../utils.ts';
import type {RGBA, ColorInput} from '../../types.ts';
import type {ParsedKey} from '../../parse.keypress.ts';
import {renderFontToFrameBuffer, measureText, fonts} from '../ascii.font.ts';
...
export class SelectElement extends BufferedElement {
  ...
  constructor(id: string, options: SelectElementOptions) {
    ...
    this.linesPerItem = this.showDescription
      ? this.font
        ? this.fontHeight + 1
        : 2
      : this.font
  ...
  protected refreshContent(
    ...
    const visibleOptions = this.options.slice(
      this.scrollOffset,
  ...
  public setSelectedIndex(index: number): void {
    if (index >= 0 && index < this.options.length) {
      ...
      this.needsRefresh = true;
      this.emit(
        SelectElementEvents.SELECTION_CHANGED,
        this.selectedIndex,
