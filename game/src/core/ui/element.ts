...
import type {BorderCharacters, BorderSides, BorderStyle, CliRenderer} from '../index.ts';
import {OptimizedBuffer} from '../buffer.ts';
import {parseColor} from '../utils.ts';
import {drawBorder} from './lib/border.ts';
import type {RGBA, ColorInput} from '../types.ts';
import type {ParsedKey} from '../parse.keypress.ts';
import {TrackedNode, createTrackedNode} from './lib/TrackedNode.ts';
import {
  FlexDirection,
  PositionType,
  Edge,
...
import type {ILayout, ILayoutElement} from './types.ts';
import {getKeyHandler, type KeyHandler} from './lib/KeyHandler.ts';
...
export interface ElementOptions
  extends Omit<RenderableOptions, 'width' | 'height'>,
...
export abstract class LayoutElement extends Renderable {
  ...
  constructor(
    id: string,
    options: Omit<RenderableOptions, 'width' | 'height'> & Partial<LayoutOptions>,
  ) {
