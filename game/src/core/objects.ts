...
} from './ui/lib/border.ts';
import {Fragment} from './styled-text.ts';
...
import {TextSelectionHelper} from './selection.ts';
      ...
      if (this.border === false) {
        ...
        if (innerEndX >= innerStartX && innerEndY >= innerStartY) {
          buffer.fillRect(
            innerStartX,
            innerStartY,
            innerEndX - innerStartX + 1,
            innerEndY - innerStartY + 1,
            this._bg,
          );
        }
      }
...
export class StyledTextRenderable extends Renderable {
  ...
  constructor(id: string, buffer: OptimizedBuffer, options: StyledTextOptions) {
    ...
    this._defaultFg = options.defaultFg
      ? parseColor(options.defaultFg)
      : RGBA.fromValues(1, 1, 1, 1);
    this._defaultBg = options.defaultBg
  ...
  private renderFragmentToBuffer(): void {
    ...
    this.frameBuffer.drawStyledTextFragment(
      ...
      this._defaultBg,
      selection
        ? {...selection, bgColor: this._selectionBg, fgColor: this._selectionFg}
        : undefined,
    );
  }
