...
export class MainMenuState extends BaseState {
    ...
    private setupEventHandlers(): void {
        if (this.tabSelect) {
            this.tabSelect.on(TabSelectElementEvents.SELECTION_CHANGED, (index: number, option: TabSelectOption) => {
                this.currentTab = option.value as string;
                this.updateContent();
            });

            this.tabSelect.on(TabSelectElementEvents.ITEM_SELECTED, (index: number, option: TabSelectOption) => {
